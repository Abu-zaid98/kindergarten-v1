            ...
            <div className="absolute left-0 right-0 top-[calc(100%+6px)] z-20 max-h-60 overflow-auto rounded-2xl border border-slate-200 bg-white p-1 shadow-xl">
              {ARABIC_MONTHS.map((name, i) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-xl px-3 py-2.5 text-right text-sm font-bold ${month === i + 1 ? 'bg-blue-50 text-blue-700' : 'text-slate-700 hover:bg-slate-50'}`}
                >
                  <span>{name}</span>
                  {month === i + 1 ? <span className="text-xs">✓</span> : null}
                </button>
              ))}
            </div>
          ) : null}
